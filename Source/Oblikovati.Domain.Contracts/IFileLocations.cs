...
public interface IFileLocations
{
    ...
    void FindLogicalInLocations(string RelativeFileName, string LibraryName, out string RepositoryName, out LocationTypeEnum Type);
    void ProjectFiles(out int NumProjectFiles, ref List<string> ProjectFileNamePaths);
    void GetVaultData(out string VaultServer, out string VaultName, out string VaultProject, out string StreamlineWatchFolder);
