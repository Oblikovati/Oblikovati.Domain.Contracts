...
public interface IBSplineCurve
{
    ...
    void GetBSplineInfo(out int Order, out int NumPoles, out int NumKnots, out bool IsRational, out bool IsPeriodic, out bool IsClosed, out bool IsPlanar, ref List<double> PlaneVector);
    void GetBSplineData(ref List<double> Poles, ref List<double> Knots, ref List<double> Weights);
    void PutBSplineInfoAndData(int Order, ref List<double> Poles, ref List<double> Knots, ref List<double> Weights, bool IsPeriodic);
    IBSplineCurve Copy();
